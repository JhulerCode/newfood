<template>
    <div
        class="modal-back"
        :class="{ 'with-back': hasBack, 'without-back': !hasBack }"
        :style="{
            'border-radius': `${hasBack == false ? borderRadius : '0'}`,
            'z-index': zIndex,
        }"
        ref="modal-back"
        @click="closeOnBack()"
...
export default {
    ...
    methods: {
        ...
        closeOnTime() {
            ...
            if (this.useModals[this.modal].closeOnSec == undefined)
                this.useModals[this.modal].closeOnSec = 3
